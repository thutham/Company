...
class Telnet(object):
    ...
    Connection` keyword. Typically the next step is logging in with `TC_Login`
    ...
    `Read Until Prompt`, and `TC_Login` keywords. The default value is 3 seconds.
    ...
    it easier, and faster, to verify did `TC_Login` succeed.
    ...
    def switch_connection(self, index_or_alias):
        ...
        | `TC_Login`             | john                    | secret   |           |
        | `Write`             | some command            |          |           |
        | `Open Connection`   | yourhost.com            | 2nd conn |           |
        | `TC_Login`             | root                    | password |           |
        | `Write`             | another cmd             |          |           |
...
class TelnetConnection(telnetlib.Telnet):
    ...
    def set_timeout(self, timeout):
        ...
        Until Regexp`, `Read Until Prompt`, `TC_Login`) use this timeout and fail
    ...
    def set_prompt(self, prompt, prompt_is_regexp=False):
        """Sets the prompt used by `Read Until Prompt` and `TC_Login` in the current connection.
    ...
    def login(self, username, password, login_prompt='login: ',
              ...
              login_incorrect='TC_Login incorrect'):
        ...
        self._log(output)
        if not success:
            raise AssertionError('TC_Login incorrect')
        return output
